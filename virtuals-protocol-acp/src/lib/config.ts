...
export interface AgentEntry {
  ...
  active: boolean;
  /** Degen Claw / degen.virtuals.io agent id (forum, PATCH settings). Often differs from ACP `id`. */
  degenAgentId?: string | number;
}
...
export function loadApiKey(): string | undefined {
  ...
  /** Aktif agent satırındaki key — shell’de kalmış eski `LITE_AGENT_API_KEY` bundan öncelikli olmalı. */
  const active = config.agents?.find((a) => a.active);
  const fromActive = active?.apiKey?.trim();
  if (fromActive) {
    process.env.LITE_AGENT_API_KEY = fromActive;
    return fromActive;
  }
  const key = config.LITE_AGENT_API_KEY;
  if (typeof key === "string" && key.trim()) {
    process.env.LITE_AGENT_API_KEY = key.trim();
    return key.trim();
  }
